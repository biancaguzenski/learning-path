...
;using set bang - a way of using mutation in racket - set redefine a variable already in use

(define d 3) ; d is 3
(define (f x) (* 1 (+ x d)))
(define c (+ d 4)) ; c is set to 7
(set! d 5) ; change d value
(define z (f 4))
(define w c) ; w is seven -> c is set t 7, wont change because of d
...
(cons (+ 7 7) #t) ; produces a pair instead of a list -> (pair?)
; improper list: cons that doesnt build a list -> list functions wont work

; mcons => optional mutable cons version -> where set works

; mcons = mutable pair
; mcar and mcdr = car/cdr
; mpair? = true if mutable pair
; set-mcar! or set-mcdr! to change components

; When are its subexpression evaluated problem - ex, functions in args are evaluated before the evaluation of the body.
; In contrast, in a inf-then-else expression, we do  not evaluate both then and else, only one
; Thunk: use (lambda () e) instead of (e), because lambda e is delayed until called, e is not