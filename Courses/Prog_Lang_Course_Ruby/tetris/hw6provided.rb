...
class Piece # Pieces of tetris + its movements
  ...
  def initialize (point_array, board)
    ...
    @color = All_Colors.sample # sample returns a random element from the awway
    @base_position = [5, 0] # [column, row] where block starts dropping
    @board = board # can't transpass
    @moved = true
  end

  def current_rotation
    @all_rotations[@rotation_index] # to rotate
  end
...
class Board
  ...
  def initialize (game)
    @grid = Array.new(num_rows) {Array.new(num_columns)} # create a matrix
...
class Tetris
  ...
  def key_bindings  
    @root.bind('n', proc {self.new_game})  # closure - call new_game if n is pressed

    @root.bind('p', proc {self.pause}) # pause or unpause if p is pressed

    @root.bind('q', proc {exitProgram}) # quit if q is pressed and so on
